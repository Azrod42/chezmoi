...
pub(crate) async fn handle(req: Request, _state: Arc<State>) -> Result<Response<Body>, Error> {
    ...
    let payload: WriterRequest = serde_json::from_slice(&body_bytes(req.body()))
    ...
    let prompt = match function {
        WriterFunction::TRANSLATE => TRANSLATE_PROMPT
            .replace("{language}", &language)
            .replace("{text}", &input),
        WriterFunction::CORRECT => CORRECT_PROMPT.replace("{text}", &input),
    };
    ...
    Ok(json(
        StatusCode::OK,
        serde_json::to_value(WriterResponse { answer }).unwrap(),
    ))
}
